...
const CAROUSEL_SLIDES = [
  ...
  { src: "/images/hero/feature-2.png", alt: "Egresados INDECAP trabajando", label: "35.000+ egresados" },
  { src: "/images/hero/feature-3.png", alt: "Prácticas reales en INDECAP", label: "Prácticas reales" },
  { src: "/images/hero/feature-4.png", alt: "Sedes INDECAP en Antioquia", label: "3 sedes en Antioquia" },
];
...
export function Hero() {
  ...
  const goTo = useCallback((index: number) => {
    ...
    setTimeout(() => { setCurrent(index); setIsTransitioning(false); }, 600);
  ...
  return (
    <section id="inicio" aria-label="Bienvenida INDECAP" className="relative flex min-h-[92vh] items-center overflow-hidden bg-[#080F14]">

      {/* CARRUSEL — aria-hidden porque es decorativo */}
      ...
      <div className="relative z-20 container mx-auto px-6 pb-16 pt-28 lg:px-12 lg:pb-24 lg:pt-32">
        <div className="max-w-3xl">
          ...
          <h1 className="mt-6 font-[family-name:var(--font-playfair)] text-[clamp(2.8rem,5vw,4.5rem)] font-black leading-tight text-white drop-shadow-lg">
            ...
            <em className="italic text-[#FFD166] drop-shadow-md">Antioquia</em>
          </h1>

          <p className="mt-6 max-w-xl text-lg font-light leading-relaxed text-gray-200">
            Corporación Educativa INDECAP. Formación técnica laboral con enfoque 100% práctico. Más de 35.000 egresados ya están trabajando en Antioquia.
          </p>
          ...
          <ul className="mt-8 flex flex-col gap-3 sm:flex-row sm:gap-6">
            {["Prácticas reales", "Alta empleabilidad", "Certificación laboral"].map((text) => (
              <li key={text} className="flex items-center gap-2 text-white/90">
                <CheckCircle className="h-5 w-5 shrink-0 text-[#FFD166]" aria-hidden="true" />
          ...
          <div className="mt-10 flex flex-wrap gap-4">
            <a
              href="https://api.whatsapp.com/send?phone=573022389760&text=Hola%20INDECAP%20quiero%20inscribirme"
              target="_blank"
              rel="noopener noreferrer"
              aria-label="Inscríbete ahora por WhatsApp"
              className="group inline-flex items-center gap-2 rounded-full bg-[#F0A500] px-8 py-4 text-sm font-bold text-[#080F14] transition-all hover:-translate-y-1 hover:shadow-[0_10px_30px_rgba(240,165,0,0.4)]"
            >
            ...
            <a
              href="#programas"
              aria-label="Ver todos los programas técnicos"
              className="inline-flex items-center gap-2 rounded-full border border-white/30 bg-white/5 px-8 py-4 text-sm font-semibold text-white backdrop-blur-sm transition-all hover:bg-white/10"
            >
              Ver programas <ChevronDown className="h-4 w-4" aria-hidden="true" />
            </a>
          </div>
        </div>
        ...
        <div className="mt-12 hidden lg:absolute lg:bottom-24 lg:right-12 lg:block">
          <dl className="flex gap-10 rounded-3xl border border-white/10 bg-white/5 p-8 shadow-2xl backdrop-blur-xl">
            {[
              ...
              <div key={stat.label} className="text-center">
                <dd className="font-[family-name:var(--font-playfair)] text-4xl font-bold" style={{ color: stat.color }}>{stat.num}</dd>
                <dt className="mt-1 text-xs font-medium uppercase tracking-wider text-gray-400">{stat.label}</dt>
              </div>
            ))}
          </dl>
        </div>
