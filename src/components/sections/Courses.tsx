        ...
        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filtrados.map((course) => {
            ...
            return (
              <div
                key={course.id}
                className="group flex flex-col overflow-hidden rounded-[20px] border border-gray-100 bg-white transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.12)]"
              >
                {/* Imagen — clickeable si tiene página */}
                {link ? (
                ...
                {/* Botones — siempre fuera de cualquier <a> o Link que envuelva */}
                <div className="flex flex-col gap-2 px-5 pb-5">
                  {link && (
                    <Link
                      href={link}
                      className="flex w-full items-center justify-center gap-2 rounded-full border py-2.5 text-xs font-bold transition-all duration-200 hover:text-white"
