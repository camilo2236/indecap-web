      ...
      <div className="container mx-auto flex h-20 items-center justify-between px-6 lg:px-12">
        ...
        <div className="flex items-center gap-4">
          {isProgram && (
            <a
              href="/#programas"
              className="flex items-center gap-1 rounded-full border border-[#E4F1F6] bg-white px-3 py-1.5 text-xs font-semibold text-[#374151] transition-all hover:border-[#312783] hover:text-[#312783]"
            >
        ...
        <nav className="hidden items-center gap-6 lg:flex">
              ...
              <div className="absolute top-full left-1/2 -translate-x-1/2 mt-3 w-[700px] bg-white rounded-2xl shadow-2xl border border-[#e4e9eb] p-6 z-50">
                <div className="grid grid-cols-3 gap-x-8 gap-y-1">
                  {escuelas.map((escuela) => (
                    <div key={escuela} className="mb-4">
                      <p
                        className="text-[10px] font-black uppercase tracking-widest mb-2 pb-1 border-b border-[#eaeff1]"
                        style={{ color: escuelaColors[escuela] }}
                      >
                        {escuela}
                      </p>
                      {programas
                        .filter((p) => p.escuela === escuela)
                        .map((prog) => (
                          <a
                            key={prog.href}
                            href={prog.href}
                            onClick={() => setDropdownOpen(false)}
                            className="block py-1.5 text-sm text-[#374151] hover:text-[#312783] hover:translate-x-1 transition-all"
                          >
                            {prog.label}
                          </a>
                        ))}
                ...
                <div className="mt-4 pt-4 border-t border-[#eaeff1] flex justify-between items-center">
                  ...
                  <a
                    ...
                    onClick={() => setDropdownOpen(false)}
                    className="text-sm font-bold text-[#805600] hover:underline"
                  >
                    Cursos y diplomados →
                  </a>
                </div>
          ...
          <a
            ...
            className={buttonVariants({
              className:
                "rounded-full bg-[#F0A500] px-7 py-3 text-sm font-black text-[#080F14] hover:bg-[#FFD166] shadow-[0_4px_20px_rgba(240,165,0,0.4)] hover:shadow-[0_4px_28px_rgba(240,165,0,0.6)] transition-all",
            })}
          >
            ...
            <nav className="mt-8 flex flex-col gap-2">
              {isProgram && (
                <a
                  ...
                  className="flex items-center gap-2 text-lg font-bold text-[#312783] mb-2"
                >
              ...
              {/* Programas acordeón móvil */}
              <button
                onClick={() => setMobileProgOpen(!mobileProgOpen)}
                className="flex items-center justify-between w-full text-lg font-medium text-[#080F14] py-2 border-b border-[#eaeff1]"
              >
                Programas
              ...
              {mobileProgOpen && (
                <div className="pl-3 pb-2">
                  {escuelas.map((escuela) => (
                    <div key={escuela}>
                      <p
                        className="text-[10px] font-black uppercase tracking-widest mt-3 mb-1"
                        style={{ color: escuelaColors[escuela] }}
                      >
                        {escuela}
                      </p>
                      {programas
                        ...
                        .map((prog) => (
                          <a
                            ...
                            className="block py-1.5 text-sm text-[#374151] hover:text-[#312783]"
                          >
                            {prog.label}
                          </a>
                        ))}
                  ...
                  <a
                    href="/programas"
                    onClick={() => { setOpen(false); setMobileProgOpen(false); }}
                    className="block mt-3 text-sm font-bold text-[#312783]"
                  >
                    Ver todos →
                  </a>
                </div>
              ...
              <a
                ...
                onClick={() => setOpen(false)}
                className={buttonVariants({
                  className: "mt-4 rounded-full bg-[#312783] px-6 py-2 font-semibold text-white",
                })}
              >
